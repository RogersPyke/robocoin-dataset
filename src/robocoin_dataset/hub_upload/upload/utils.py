...
def create_upload_config(config_dict: dict) -> UploadConfig:
    """
    ...
    return UploadConfig(
        ...
        pg_cfg_oath=config_dict.get("db_file_path", ""),
        skip_err=config_dict.get("skip_missing", True),
        upload_force_overwrite=config_dict.get("force_overwrite", False),
        upload_readme_only=config_dict.get("readme_only", False),
    )
...
class LocalDsUploadUtil(LocalDsUtil):
    ...
    def __init__(self, config: UploadConfig) -> None:
        ...
        if self.config.pg_cfg_oath:
            try:
                self.metadata_service = MetadataSyncService(
                    db_file_path=self.config.pg_cfg_oath,
                    logger=self.logger,
                )
    ...
    def _upload_one_dataset(
        ...
        if self.config.upload_readme_only:
